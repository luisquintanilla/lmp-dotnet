...
- [x] Create `PromptBuilderEmitter` that generates `file static class {TypeName}PromptBuilder`:
  ...
  - `private static string FormatOutput(TOutput output)` — JSON-serializes (uses default serializer; Task 2.4 will add generated JsonContext)
...
- [x] Added `InputTypeName` and `InputTypeFullyQualifiedName` properties to `OutputTypeModel` — emitter skips emission when null
- [x] Wired `PromptBuilderEmitter.Emit()` into `LmpSourceGenerator` pipeline for valid models with input type resolved
- [x] Unit tests: 26 tests in `PromptBuilderEmitterTests` — structure, syntax validity, escaping, field descriptions, all methods

**Status:** ✅ Complete. 140 total tests pass (51 Abstractions + 16 Core + 73 SourceGen). Emitter is fully implemented and tested. Pipeline emits PromptBuilder when `InputTypeName` is set; this will be populated when Predictor\<TIn,TOut\> discovery is wired in Task 2.5. `FormatOutput` uses default `JsonSerializer.Serialize()` — Task 2.4 will switch to generated `JsonContext`.

**Completion criteria:** ~~`dotnet build` produces `ClassifyTicket.PromptBuilder.g.cs` with correct prompt assembly.~~ Emitter generates correct source (verified by 26 unit tests including syntax validation). Pipeline emission deferred until `InputTypeName` is resolved from Predictor discovery (Task 2.5).
