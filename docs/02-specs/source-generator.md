...
## 4. Artifact 2 — `JsonSerializerOptions` per Module

### What It Does

Emits a per-module `JsonSerializerOptions` instance configured with `camelCase` naming and null-ignoring. This is wired into each predictor's `SerializerOptions` property so JSON serialization (for few-shot demos, structured output parsing, etc.) uses consistent settings.

> **Important limitation:** Roslyn source generators run independently and in parallel. A source generator **cannot** emit a `partial class : JsonSerializerContext` and expect the STJ (System.Text.Json) source generator to process it — each generator only sees the original source code, not output from other generators. LMP therefore emits plain `JsonSerializerOptions`, not a `JsonSerializerContext`.

### Generated Code

For `TicketTriageModule`, the generator emits `TicketTriageModule.JsonOptions.g.cs`:
...
[GeneratedCode("LMP.Generators", "1.0.0")]
internal static class TicketTriageModuleJsonOptions
{
...
The generated `GetPredictors()` method (see Artifact 3) wires `SerializerOptions ??= TicketTriageModuleJsonOptions.Instance` on each predictor, so serialization settings are applied without any user configuration.

### Why This Matters

| Without source gen | With source gen |
|---|---|
| Each `JsonSerializer.Serialize<T>()` call uses default options | Consistent `camelCase` + null-ignoring options across all predictors |
| User must manually configure `JsonSerializerOptions` | Options auto-wired by generated code |
| No path for AOT consumers to inject their own `JsonSerializerContext` | `predictor.SerializerOptions` is settable — AOT consumers can provide a `[JsonSerializable]`-backed context's `.Options` |

### Full AOT with User-Provided Context

For consumers publishing with `PublishAot`, the source-generated `JsonSerializerOptions` avoids most trim warnings. For **full** AOT safety (zero trim warnings), the consumer provides their own `JsonSerializerContext`:

```csharp
[JsonSerializable(typeof(TicketInput))]
[JsonSerializable(typeof(ClassifyTicket))]
[JsonSerializable(typeof(DraftReply))]
internal partial class MyAotContext : JsonSerializerContext;

// Wire it into the module's predictors
module.GetPredictors().ForEach(p => p.Value.SerializerOptions = MyAotContext.Default.Options);
```
...
partial class TicketTriageModule
{
    ...
    {
        _classify.SerializerOptions ??= TicketTriageModuleJsonOptions.Instance;
        _draft.SerializerOptions ??= TicketTriageModuleJsonOptions.Instance;
        return
        [
            ("_classify", _classify),
            ("_draft", _draft),
